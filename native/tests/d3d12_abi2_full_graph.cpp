...
int main() try {
    const char* model_environment = std::getenv("LOTUS_MODEL");
    ...
    if ((capabilities.flags & required) != required)
        throw std::runtime_error("Lotus ABI2 GPU capability is incomplete");
    ...
    const char* prompt_environment = std::getenv("LOTUS_PROMPT_CACHE");
    ...
    check_counter(lotus_get_transfer_counters(
    ...
    if (submit_ms > 5.0) throw std::runtime_error("Lotus submit exceeded 5 ms");
    ...
    if (status.state != IBRH_JOB_COMPLETE || status.source_frame_id != 7007u)
        throw std::runtime_error("Lotus ABI2 job did not complete with correlation");
    ...
    if (upload_after != upload_before || download_after != download_before)
        throw std::runtime_error(
            "Lotus external path staged tensor bytes through host");
    ...
    if (finite_count < gpu.size() / 2u)
        throw std::runtime_error("Lotus external output has too few valid mask pixels");
    ...
    if (!(maximum > minimum))
        throw std::runtime_error(
            "Lotus external output is not varying: min=" +
            std::to_string(minimum) + " max=" + std::to_string(maximum));
    double maximum_submit_ms = submit_ms;
    const char* stress_environment = std::getenv("LOTUS_STRESS_FRAMES");
    ...
    for (std::uint64_t frame = 7008u;
        ...
        if (status.state != IBRH_JOB_COMPLETE || status.source_frame_id != frame)
            throw std::runtime_error("Lotus stress correlation failed");
        wait_fence(selected.device.Get(), ready);
    }
    api.job_release(job);
    close_capture(source); close_output(output);
    api.model_unload(model); api.runtime_destroy(runtime);
    std::cout << "Lotus ABI2 D3D12/Vulkan passed; frames=" << total_frames
