...
int main() {
    const char* path = std::getenv("LOTUS_SNAPSHOT");
    const char* prompt = std::getenv("LOTUS_PROMPT_CACHE");
    const char* variant = std::getenv("LOTUS_VARIANT");
    if (path == nullptr || *path == '\0' ||
        prompt == nullptr || *prompt == '\0') {
        std::cout <<
            "LOTUS_SNAPSHOT/LOTUS_PROMPT_CACHE is not set; skipping\n";
        return 77;
    }
    ibrh_api api{};
    if (!check(
