...
void inferbridge_shape(
    ...
    if (width == 0u || height == 0u)
        throw std::invalid_argument("Lotus image dimensions are empty");
...
void validate_input(
    ...
    check_hresult(device->OpenSharedHandle(
        ...
        IID_PPV_ARGS(&resource)), "OpenSharedHandle(Lotus input)");
    ...
    if (description.Dimension != D3D12_RESOURCE_DIMENSION_TEXTURE2D ||
        ...
        throw std::invalid_argument("shared Lotus input texture is invalid");
}
...
std::shared_ptr<GpuSlot> acquire_slot(
    ...
    throw std::runtime_error("all Lotus GPU output slots are occupied");
}
...
public:
    ExternalJobImpl(
        ...
        ExternalTextureRequest request, std::uint64_t fence_value)
    ...
    ExternalTextureOutput output() const override {
        if (cancelled_.load()) throw std::runtime_error("Lotus job cancelled");
        return {
    ...
    std::shared_ptr<ExternalJob> submit_texture(
...
#if !defined(_WIN32)
        (void)request;
        throw std::runtime_error("Lotus D3D12 interop is unavailable");
#else
        if (!capabilities().available)
            throw std::runtime_error("Lotus external GPU path is unavailable");
        if (!request.shared_texture_handle || !request.wait_fence_handle ||
            !request.width || !request.height)
            throw std::invalid_argument("invalid Lotus external GPU request");
        ...
        try {
            ...
            return std::make_shared<ExternalJobImpl>(
                ...
                std::move(submission), request, signal_value);
        } catch (...) {
