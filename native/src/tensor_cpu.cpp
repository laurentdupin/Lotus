...
const TensorView& vector_tensor(
    ...
    if (tensor.rank != 1 || tensor.dimensions[0] != count) {
        throw std::runtime_error("Lotus vector shape mismatch: " + name);
    }
    return tensor;
}
...
ImageTensor conv2d(
    ...
    if (weight.rank != 4 ||
        ...
        throw std::runtime_error(
            "Lotus convolution shape mismatch: " + weight_name);
    }
    ...
    if (input.height + pad_before + pad_after < kernel ||
        ...
        throw std::runtime_error("Lotus convolution input is too small");
    }
    ImageTensor output{
        output_channels,
...
void group_norm(
    ...
    if (tensor.channels % groups != 0) {
        throw std::runtime_error("Lotus group norm channel mismatch");
    }
...
void add_in_place(ImageTensor& destination, const ImageTensor& source) {
    if (destination.channels != source.channels ||
        ...
        throw std::runtime_error("Lotus residual shape mismatch");
    }
    parallel_for(
...
ImageTensor spatial_attention(
    ...
    const auto project = [&](const char* suffix, std::vector<float>& out) {
        ...
        if (weight.rank != 2 ||
            ...
            throw std::runtime_error(
                "Lotus attention projection shape mismatch: " + base);
        }
