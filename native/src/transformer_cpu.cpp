...
void add(TokenTensor& destination, const TokenTensor& source) {
    if (destination.tokens != source.tokens ||
        ...
        throw std::runtime_error("Lotus token residual shape mismatch");
    }
    parallel_for(
...
TokenTensor layer_norm(
    ...
    if (weight.rank != 1 ||
        ...
        throw std::runtime_error("Lotus layer norm shape mismatch");
    }
...
TokenTensor attention(
    ...
    if (q.dimensions != k.dimensions ||
        ...
        throw std::runtime_error("Lotus attention head mismatch");
    }
...
TokenTensor feed_forward(
    ...
    if (projected.dimensions % 2 != 0) {
        throw std::runtime_error("Lotus GEGLU dimension mismatch");
    }
...
TokenTensor linear(
    ...
    if (weight.rank != 2 ||
        ...
        throw std::runtime_error(
            "Lotus linear shape mismatch: " + weight_name);
    }
    ...
    if (!bias_name.empty()) {
        ...
        if (bias_view.rank != 1 ||
            ...
            throw std::runtime_error(
                "Lotus linear bias mismatch: " + bias_name);
        }
        bias = bias_view.data;
    }
    TokenTensor output{
