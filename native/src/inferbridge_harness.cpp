...
public:
    ...
    void retire(std::shared_ptr<lotus_native::ExternalJob> job) {
        {
            std::lock_guard<std::mutex> lock(mutex_);
...
private:
    void run() noexcept {
        for (;;) {
            ...
            bool stopping = false;
            std::deque<std::shared_ptr<lotus_native::ExternalJob>> retired;
            {
            ...
            try {
                ...
                std::fprintf(stderr, "Lotus GPU submission failed: %s\n",
                    error.what());
                {
                    std::lock_guard<std::mutex> lock(job->gpu_mutex);
    ...
    std::deque<std::shared_ptr<lotus_native::ExternalJob>> retired_;
    ...
    *output = nullptr;
    ...
    if (request->input_count != 1u || !request->inputs ||
        ...
        return fail(model->runtime, IBRH_ERROR_INVALID_ARGUMENT,
                    "Lotus requires one input and one Core-owned output");
    ...
    if (destination.width != planned_width || destination.height != planned_height ||
        ...
        return fail(model->runtime, IBRH_ERROR_INVALID_ARGUMENT,
                    "Lotus Core-owned output does not match the output plan");
    ...
    if (input.domain != IBRH_RESOURCE_DOMAIN_HOST ||
        ...
        return fail(model->runtime, IBRH_ERROR_UNSUPPORTED_CAPABILITY,
                    "Lotus host transfer bindings are invalid");
    auto* job = new (std::nothrow) ibrh_job();
...
struct LinuxCaptureHooks {
    static ibr_linux_capture_capabilities capabilities(ibrh_model* model) {
        std::lock_guard<std::mutex> lock(model->submit_mutex);
        return lotus_linux_capture_capabilities(model->context);
    }
    static void infer(ibrh_model* model,const inferbridge::linux_capture::LinuxDmaBufImage& source,
        ...
        lotus_infer_linux_capture(model->context,source,seed,output);
    }
};
}
