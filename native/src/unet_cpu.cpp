...
ImageTensor concatenate(
    ...
    if (left.height != right.height || left.width != right.width) {
        throw std::runtime_error("Lotus skip shape mismatch");
    }
    ImageTensor output{
...
ImageTensor unet_resnet(
    ...
    if (time_hidden.dimensions != hidden.channels) {
        throw std::runtime_error("Lotus time projection shape mismatch");
    }
...
ImageTensor unet_predict(
    ...
    if (time.values.size() != class_embedding.values.size()) {
        throw std::runtime_error("Lotus class embedding mismatch");
    }
    ...
    for (std::uint32_t block = 0; block < 4; ++block) {
        for (std::uint32_t layer = 0; layer < 3; ++layer) {
            if (skips.empty()) {
                throw std::runtime_error("Lotus skip stack underflow");
            }
        ...
        if (block != 3) {
            if (skips.empty()) {
                throw std::runtime_error(
                    "Lotus skip stack lacks upsample target");
            }
    ...
    if (!skips.empty()) {
        throw std::runtime_error("Lotus skip stack was not consumed");
    }
    group_norm(
