...
vec4 rw4(uint n){
 vec2 a=unpackHalf2x16(w.d[n>>1]);
 vec2 b2=unpackHalf2x16(w.d[(n>>1)+1]);
 return vec4(a,b2);
}
...
void main(){
 ...
 for(uint off=0;off<4;++off){uint ch=cb+off;if(ch<p.oc)
  o.d[ob+(ch*p.oh+y)*p.ow+x]=s0[off]+(p.has_bias!=0?b.d[ch]:0);}
 for(uint off=0;off<4;++off){uint ch=cb+4+off;if(ch<p.oc)
  o.d[ob+(ch*p.oh+y)*p.ow+x]=s1[off]+(p.has_bias!=0?b.d[ch]:0);}
}
