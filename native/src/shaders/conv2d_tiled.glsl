...
shared float st[100];shared float kt[36];
void main(){
 ...
 float s[4]=float[4](0,0,0,0);uint lane=gl_LocalInvocationID.y*8+gl_LocalInvocationID.x;
 ...
 for(uint c=0;c<p.ic;++c){
  for(uint n=lane;n<100;n+=64){int px=ox+int(n%10),py=oy+int(n/10);
   st[n]=px>=0&&px<int(p.iw)&&py>=0&&py<int(p.ih)?i.d[ib+(c*p.ih+uint(py))*p.iw+uint(px)]:0;}
  for(uint n=lane;n<36;n+=64){uint off=n/9,ch=cb+off;
   kt[n]=ch<p.oc?w.d[(ch*p.ic+c)*9+n%9]:0;}
  barrier();if(valid)for(uint ky=0;ky<3;++ky)for(uint kx=0;kx<3;++kx){
   float v=st[(gl_LocalInvocationID.y+ky)*10+gl_LocalInvocationID.x+kx];uint k=ky*3+kx;
   for(uint off=0;off<4;++off)s[off]+=v*kt[off*9+k];}barrier();
 }
 if(!valid)return;for(uint off=0;off<4;++off){uint ch=cb+off;if(ch<p.oc)
